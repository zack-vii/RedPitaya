...
struct rp_timing *rp_timing_get_device(const char *dev_file)
{
	...
	if(!dev)
	{
		...
		fprintf(stderr, "trying to allocate %u bytes of data\n", (uint32_t)sizeof(struct rp_timing));
		dev = mmap(NULL, sizeof(struct rp_timing), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
	}
	if(!dev)
	{
...
int rp_timing_release_device()
{
	struct rp_timing *dev = rp_timing_get_device(0);
	int c_status;
	if(dev)
	{// release
		c_status = munmap(dev, sizeof(struct rp_timing));
